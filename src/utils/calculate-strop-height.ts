import { launchProfile, generateDynamicLaunchProfile } from "./launch-profile";
...
export const calculateStropHeights = (
  ...
  } else if (upperWindSpeed !== undefined) {
    // Use dynamic launch profile if upper wind speed is provided
    const dynamicProfileData = generateDynamicLaunchProfile(windSpeed, upperWindSpeed);
    closestProfile = { 
      wind: `dynamic-${windSpeed}-${upperWindSpeed}`, 
      data: dynamicProfileData 
    };
  } else {
    // Find the closest wind speed profile from static profiles
